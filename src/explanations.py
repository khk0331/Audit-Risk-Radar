...
def recommend_audit_steps(row: pd.Series) -> list[str]:
    ...
    if not triggered:
        return [
            "[재무비율 조합] 복수 지표가 함께 움직인 원인을 파악하기 위해 전년 대비 주요 재무비율 변동 원인을 경영진 설명과 대조합니다. (근거: ISA 315/520 · ISA 315는 회사와 환경을 이해해 중요한 왜곡표시위험을 식별·평가하도록 하고, ISA 520은 재무정보 간 그럴듯한 관계를 평가하는 분석적 절차를 요구합니다.)",
            "[Peer 비교] 동일 Industry 내 유사 기업과 비교하여 해당 변동이 산업 전반의 현상인지 개별 회사 고유 리스크인지 구분합니다. (근거: ISA 315/520 · ISA 520은 예상값과 다른 유의적 변동이나 관계를 조사하도록 합니다.)",
            "[후속 절차 설계] Final Risk가 높은 경우 주석 공시와 사업보고서 내 경영진 설명을 더 자세히 확인하고, 내부자료 접근이 가능한 감사 단계에서는 관련 계정 영역을 후속 확인 후보로 둡니다. (근거: ISA 330/500 · ISA 330은 평가된 위험에 대응하는 절차 설계를, ISA 500은 충분하고 적합한 감사증거 확보를 요구합니다.)",
        ]
