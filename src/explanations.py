...
FEATURE_DETAIL = {
    ...
    "tata": {
        ...
        "risk": "회계상 이익이 영업현금흐름으로 충분히 뒷받침되는지, 매출채권·재고·충당부채 같은 운전자본 및 추정 계정 변화와 함께 확인해야 합니다.",
    },
}
...
AUDIT_FOCUS = {
    "dsri": {
        "area": "매출채권 및 수익 인식",
        "question": "매출 증가가 실제 현금 회수로 이어지고 있는가?",
        "procedure": "공시 재무제표에서 매출 증가율, 매출채권 증가율, 영업현금흐름, 대손충당금 변동을 같은 표로 비교합니다. 매출채권이 매출보다 빠르게 늘면 주석에서 신용정책, 주요 거래처, 대손충당금 설명을 우선 확인합니다.",
        "basis": "ISA 315/330/500/520, IFRS 15",
        "isa_quote": "ISA 520은 재무정보 간 그럴듯한 관계를 평가하는 분석적 절차를 요구합니다.",
    },
    "gmi": {
        "area": "매출총이익률 및 원가 구조",
        "question": "매출총이익률 악화가 매출원가·재고·가격 정책 변화로 설명되는가?",
        "procedure": "공시 재무제표에서 매출총이익률, 매출원가율, 재고자산 회전, 재고평가손실 관련 주석을 전년 및 peer와 비교합니다. 원가율 상승 설명이 부족하면 수익성 악화와 재고평가 이슈를 함께 표시합니다.",
        "basis": "ISA 315/330/500/520, IAS 2",
        "isa_quote": "ISA 315는 회사와 환경을 이해해 중요한 왜곡표시위험을 식별·평가하도록 합니다.",
    },
    "aqi": {
        "area": "자산 건전성",
        "question": "기타·비유동성 자산 증가가 사업 확장으로 설명되는가, 아니면 자산화/손상 이슈가 있는가?",
        "procedure": "공시 재무제표에서 무형자산, 유형자산, 기타비유동자산 증가율과 손상차손·개발비·사용권자산 주석을 확인합니다. 자산 증가가 매출·영업현금흐름과 동행하지 않으면 손상검토 초점으로 표시합니다.",
        "basis": "ISA 315/330/500, IAS 16/36/38",
        "isa_quote": "ISA 500은 충분하고 적합한 감사증거를 얻기 위한 절차 설계를 요구합니다.",
    },
    "sgi": {
        "area": "고성장 매출",
        "question": "이례적인 매출 성장이 현금흐름과 매출채권 변화로도 뒷받침되는가?",
        "procedure": "공시 재무제표에서 매출 성장률, 영업현금흐름 증가율, 매출채권/매출 비율, 주요 고객·사업부문 주석을 함께 확인합니다. 고성장인데 현금흐름이 약하면 수익 인식 설명을 우선 검토합니다.",
        "basis": "ISA 240/315/330/500/520, IFRS 15",
        "isa_quote": "ISA 240은 부정으로 인한 중요한 왜곡표시위험을 고려할 때 전문가적 의구심을 요구합니다.",
    },
    "sgai": {
        "area": "판관비 및 영업비용",
        "question": "판관비 부담 증가가 일회성 비용인지, 지속적인 수익성 저하 신호인지 구분되는가?",
        "procedure": "공시 재무제표에서 판관비율, 영업이익률, 인건비·광고선전비·연구개발비 주석을 전년 및 peer와 비교합니다. 비용 증가 설명이 없으면 비용 구조 악화와 자본화 가능성을 구분해 표시합니다.",
        "basis": "ISA 315/330/500/520, IAS 1",
        "isa_quote": "ISA 520은 예상값과 다른 유의적 변동이나 관계를 조사하도록 합니다.",
    },
    "lvgi": {
        "area": "레버리지 및 유동성",
        "question": "부채 증가가 유동성, 이자비용, 계속기업 관련 공시로 설명되는가?",
        "procedure": "공시 재무제표에서 부채비율, 유동비율, 차입금 만기, 이자비용, 현금및현금성자산, 계속기업 관련 주석을 확인합니다. 단기차입금 증가와 현금흐름 약화가 동시에 있으면 유동성 초점으로 표시합니다.",
        "basis": "ISA 315/330/500/570, IAS 1/IFRS 7",
        "isa_quote": "ISA 570은 계속기업 가정과 관련된 감사인의 책임을 다룹니다.",
    },
    "tata": {
        "area": "이익-현금흐름 괴리",
        "question": "순이익과 영업현금흐름의 괴리가 정상적인 운전자본 변화로 설명되는가?",
        "procedure": "공시 재무제표에서 순이익, 영업현금흐름, 매출채권, 재고자산, 매입채무, 충당부채 변동을 함께 확인합니다. 이익은 증가했지만 영업현금흐름이 약하면 어떤 운전자본 항목이 차이를 만든 것인지 표시합니다.",
        "basis": "ISA 315/330/500/520/540, IAS 37",
        "isa_quote": "ISA 540은 회계추정과 관련 공시의 중요한 왜곡표시위험 대응을 요구합니다.",
    },
}
...
def get_triggered_features(row: pd.Series) -> list[str]:
    ...
    # planning trigger, while TATA uses an earnings-cash-flow gap threshold.
    triggered = []
...
def explain_accounting_layer(row: pd.Series) -> str:
    ...
    lines = [
        ...
        f"**M-Score 관점:** M-Score는 {_format_number(row.get('m_score'))}입니다. 전통적으로 `-2.22`보다 높으면 주의 신호로 보지만, 여기서는 단독 판단이 아니라 매출채권, 수익성, 이익-현금흐름 괴리, 레버리지 신호를 함께 읽기 위한 기준점입니다.",
    ]
    ...
    lines.append(
        "**감사적 의미:** 이 점수가 높으면 매출 인식, 매출채권 회수가능성, 자산화/손상, 이익과 영업현금흐름의 차이처럼 경영진 판단이 개입되는 계정 영역을 더 깊게 이해해야 합니다. 공시자료 단계에서는 결론보다 `어느 계정에 질문을 던질지`를 정하는 신호로 사용하는 것이 적절합니다."
    )
    return "\n".join(lines)
...
def detailed_risk_analysis(row: pd.Series) -> str:
    ...
    summary.extend(
        [
            ...
            "**감사계획 시사점:** 공시자료만으로 원장·전표 수준의 결론을 낼 수는 없습니다. 대신 이 분석은 매출채권, 수익 인식, 이익-현금흐름 괴리, 자산 건전성, 레버리지 중 어느 영역을 먼저 질문할지 정리해주는 사전 검토 도구입니다.",
        ]
    )
    return "\n".join(summary)
